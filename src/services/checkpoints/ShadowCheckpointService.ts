...
export abstract class ShadowCheckpointService extends EventEmitter {
	...
	public async initShadowGit() {
		...
		if (await fileExistsAtPath(this.dotGitDir)) {
			...
			this.baseHash = await git.revparse(["--abbrev-ref", "HEAD"])
		} else {
		...
		this.log(`[${this.constructor.name}#initShadowGit] initialized shadow git in ${duration}ms`)
	...
	public async saveCheckpoint(message: string): Promise<CheckpointResult | undefined> {
		try {
			...
			const duration = Date.now() - startTime
			this.log(`[${this.constructor.name}#saveCheckpoint] checkpoint saved in ${duration}ms -> ${toHash ?? "-"}`)
